...
window.onload = async () => {
    ...
    if (Array.isArray(window.serverAdd)) {
        ...
        util.pullJSON("gamemodeData").then((json) => {
            document.getElementById("serverName").innerHTML = `<h4 class="nopadding">${json.gameMode} | ${json.players}/40</h4>`;
        });
    }
    // Save forms
...
function drawMinimapAndDebug(spacing, alcoveSize, GRAPHDATA) {
    ...
    if (global.mspt >= 50) orangeColor = true;
    if (global.metrics.rendertime < 10) orangeColor = true;
    // Text
    if (global.showDebug) {
        drawText("unnamed old growth game", x + len, y - 50 - 5 * 14 - 2, 15, color.guiwhite, "right");
        ...
        drawText("unnamed old growth game", x + len, y - 50 - 2 * 14 - 2, 15, color.guiwhite, "right");
    ...
    } else drawText("unnamed old growth game", x + len, y - 22 - 2 * 14 - 2, 15, color.guiwhite, "right");
}
