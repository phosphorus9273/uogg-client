...
const util = {
    ...
    pullJSON: (fileName, ip) => {
        return new Promise((resolve, reject) => {
            let url = `${location.protocol}//${ip}/lib/json/${fileName}.json`;
            if (ip === "localhost" || ip === "phosphorus.dakarr.cc") {
                url = `http://${ip}/lib/json/${fileName}.json`;
            }
            ...
            fetch(url).then(response => {
                ...
                console.log("JSON load from " + url + " failed: ", error);
                reject(error);
            });
        });
    },    
