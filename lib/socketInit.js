...
const convert = {
  ...
  data: () => {
    ...
    // Manage dead or leftover entities efficiently
    for (let i = 0; i < global.entities.length; i++) {
        const e = global.entities[i];
        
        // Set entity status based on health
        e.render.status.set(e.health === 1 ? "dying" : "killed");
        
        // Only push if not entirely dead and still visible
        if (
            e.render.status.getFade() !== 0 &&
            util.isInView(
                e.render.x - global.player.renderx,
            ...
            // Remove text objects if the entity is fully "dead"
            if (e.render.textobjs) {
                for (let o of e.render.textobjs) {
                    o.remove();
                }
            }
    ...
    // Update and sort global entities
    global.entities = output;
    global.entities.sort((a, b) => {
        const layerDiff = a.layer - b.layer;
        return layerDiff !== 0 ? layerDiff : b.id - a.id;
    });
},
...
const socketInit = (port) => {
  ...
  if (window.serverAdd.includes('localhost')) {
    ...
    socketProtocol = protocols[location.protocol];
  }

// Initialize WebSocket connection
let socket = new WebSocket(socketProtocol + (window.serverAdd ? window.serverAdd : 'wg.dakarr.cc'));
  // Set up our socket
